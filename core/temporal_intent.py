...
COMPARISON_PATTERNS = [
    ...
    r'\b(since|from)\s+(the\s+)?(pre-?(?:pandemic|covid)|post-?(?:pandemic|covid)|covid|pandemic|great\s+recession|\d{4})',

    # "compared to X" / "vs X"
    r'\b(compared?\s+to|versus|vs\.?)\s+(the\s+)?(pre-?(?:pandemic|covid)|post-?(?:pandemic|covid)|covid|pandemic|great\s+recession|\d{4}|last\s+year|a\s+year\s+ago)',
    ...
    r'\b(higher|lower|more|less|better|worse|stronger|weaker)\s+than\s+(the\s+)?(pre-?(?:pandemic|covid)|before|in\s+\d{4})',

    # "recovery from X"
    r'\brecovery\s+(from|since)\s+',

    # "X vs Y" where both are temporal
    r'\b(\d{4})\s+(vs\.?|versus|compared\s+to)\s+(\d{4})',
    ...
    r'\bcompare\s+.+\s+to\s+(the\s+)?(pre-?(?:pandemic|covid)|post-?(?:pandemic|covid)|\d{4})',
]
...
FILTER_PATTERNS = [
    ...
    r'\bduring\s+(the\s+)?(covid|pandemic|great\s+recession|\d{4})(?!\s+(vs|versus|compared))',
...
def _extract_reference_period(query_lower: str) -> Optional[dict]:
    ...
    year_patterns = [
        r'(?:since|from|compared\s+to|vs\.?|versus)\s+(\d{4})',
        r'(\d{4})\s+levels?',
...
def get_reference_period_bounds(period_name: str) -> Optional[dict]:
    ...
    Args:
        period_name: Name like "pre-pandemic", "covid", "great-recession", or a year
